...
def Suspend(name, pattern=SleepPatterns.Suspend):
    return BasicLog(name, pattern)

def Active(name, pattern=SleepPatterns.Active):
    return BasicLog(name, pattern)

def SleepCycle(name, pattern=SleepPatterns.SleepCycle):
    return BasicLog(name, pattern)


class Ignition(BasicLog):
    def __init__(self, name, pattern=SleepPatterns.IGN):
        super(Ignition, self).__init__(name, pattern)
        self.result_dict.IGN = EasyDict(
            {'IGN_ON1': EasyDict(start=None, duration=None),
             'IGN_OFF1': EasyDict(start=None, duration=None)}
        )
    ...
    def update_counts(self, match):
        ...
        self.result_dict.IGN[f"IGN_OFF{self.off_counter}"] = self.result_dict.IGN.get(f"IGN_OFF{self.off_counter}",  EasyDict(start=None, duration=None))
        self.result_dict.IGN[f"IGN_ON{self.on_counter}"] = self.result_dict.IGN.get(f"IGN_ON{self.on_counter}",  EasyDict(start=None, duration=None)) 

        IgnOff = self.result_dict.IGN[f"IGN_OFF{self.off_counter}"]
        IgnOn = self.result_dict.IGN[f"IGN_ON{self.on_counter}"]

        if status == 0:
            IgnOff.start = start_time.strftime("%d %B %H:%M") 
            IgnOn.duration = self.time_difference(IgnOn.start, start_time)
            self.off_counter += 1
        elif status == 1:
            IgnOn.start = start_time.strftime("%d %B %H:%M")
